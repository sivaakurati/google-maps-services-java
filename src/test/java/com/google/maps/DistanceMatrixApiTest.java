...
public class DistanceMatrixApiTest {
	...
	public void testGetDistanceMatrixWithBasicStringParams() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(getDistanceMatrixWithBasicStringParams)) {
			...
			// Rows length will match the number of origin elements, regardless of whether
			// they're
			// routable.
	 ...
	 *
	 * <p>
	 * Sample request: <a href=
	 * "http://maps.googleapis.com/maps/api/distancematrix/json?origins=Vancouver+BC|Seattle&destinations=San+Francisco|Victoria+BC&mode=bicycling&language=fr-FR">
	 * origins: Vancouver BC|Seattle, destinations: San Francisco|Victoria BC, mode:
	 * bicycling, language: french</a>.
	 */
	@Test
	...
	public void testDurationInTrafficWithTrafficModel() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext("{\"status\" : \"OK\"}")) {
			...
			DistanceMatrixApi.newRequest(sc.context).origins("Fisherman's Wharf, San Francisco")
					...
					.departureTime(new DateTime(System.currentTimeMillis() + ONE_HOUR_MILLIS)).await();
