...
public class GeocodingApiTest {
	 ...
	 * Searching for a route of Annegatan, in the administrative area of Helsinki,
	 * and the country of Finland </a>.
	 */
	@Test
	...
	public void testUtfResult() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(utfResultGeocodeResponse)) {
			...
			assertEquals("1 Rue Fernand Raynaud, 36000 ChÃ¢teauroux, France", results[0].formattedAddress);
	 ...
	 * Address Geocoding in the Google Maps APIs</a> for the reasoning behind this
	 * usage.
	 */
	@Test
	...
	public void testReverseGeocodeWithKitaWard() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(reverseGeocodeWithKitaWardResponse)) {
			...
			assertNotNull(Arrays.toString(results));
			assertEquals(
					"Japan, ã€’603-8361 KyÅ�to-fu, KyÅ�to-shi, Kita-ku, KinkakujichÅ�, ï¼‘ åŒ—å±±é¹¿è‹‘å¯ºé‡‘é–£å¯º",
					results[0].formattedAddress);
