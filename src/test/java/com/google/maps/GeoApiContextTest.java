...
public class GeoApiContextTest {
	...
	public void testGetIncludesDefaultUserAgent() throws Exception {
		...
		for (String headerName : headers.names()) {
			if (headerName.equals("User-Agent")) {
				...
				assertTrue("User agent not in correct format", headerValue.matches("GoogleGeoApiClientJava/[^\\s]+"));
			}
		}
	...
	public void testErrorResponseRetries() throws Exception {
		...
		// Execute
		GeocodingResult[] result = builder.build().get(new ApiConfig("/"), GeocodingApi.Response.class, "k", "v")
				.await();
		assertEquals(1, result.length);
		assertEquals("1600 Amphitheatre Parkway, Mountain View, CA 94043, USA", result[0].formattedAddress);
	...
	public void testSettingMaxRetries() throws Exception {
		...
		// This should limit the number of retries, ensuring that the success response
		// is NOT returned.
	...
	public void testRetryCanBeDisabled() throws Exception {
		...
		// This should disable the retry, ensuring that the success response is NOT
		// returned
	...
	public void testQueryParamsHaveOrderPreserved() throws Exception {
		// This test is important for APIs (such as the speed limits API) where multiple
		// parameters
		...
		setMockBaseUrl();
		builder.build().get(new ApiConfig("/"), GeocodingApi.Response.class, "a", "1", "a", "2", "a", "3")
				.awaitIgnoreError();
	...
	public void testToggleIfExceptionIsAllowedToRetry() throws Exception {
		// Enqueue some error responses, although only the first should be used because
		// the response's
