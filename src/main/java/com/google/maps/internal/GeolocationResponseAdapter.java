...
public class GeolocationResponseAdapter extends TypeAdapter<GeolocationApi.Response> {
	 ...
	 * </pre>
	 *
	 * Error Case: The response contains an object with a single error object with
	 * the following keys:
	 *
	 * <p>
	 * code: This is the same as the HTTP status of the response. {@code message}: A
	 * short description of the error. {@code errors}: A list of errors which
	 * occurred. Each error contains an identifier for the type of error (the
	 * reason) and a short description (the message). For example, sending invalid
	 * JSON will return the following error:
	 *
	 * <pre>
	 *   {
	 *     "error": {
	...
	public GeolocationApi.Response read(JsonReader reader) throws IOException {
		...
		while (reader.hasNext()) {
			...
			// two different objects could be returned a success object containing
			// "location" and
