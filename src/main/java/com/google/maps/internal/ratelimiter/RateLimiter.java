 ...
 * {@code acquire(1)} and an invocation to {@code
 * acquire(1000)} will result in exactly the same throttling, if any), but it
 * affects the throttling of the <i>next</i> request. I.e., if an expensive task
 * arrives at an idle RateLimiter, it will be granted immediately, but it is the
 * <i>next</i> request that will experience extra throttling, thus paying for
 * the cost of the expensive task.
 *
 * <p>
...
public abstract class RateLimiter {
	 ...
	 * every {@code (1.0 / permitsPerSecond)} seconds. When the rate limiter is
	 * unused, bursts of up to {@code permitsPerSecond} permits will be allowed,
	 * with subsequent requests being smoothly limited at the stable rate of
	 ...
	 * @throws IllegalArgumentException
	 *             if {@code permitsPerSecond} is negative or zero or {@code
	 ...
	 * after an invocation to {@code
	 * setRate} will not be affected by the new rate; it will pay the cost of the
	 * previous request, which is in terms of the previous rate.
	 *
	 * <p>
	 ...
	 * a warmup period of 20 seconds after this method invocation.
	 *
	 * @param permitsPerSecond
