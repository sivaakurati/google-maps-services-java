...
/**
 * A PendingResult backed by a HTTP call executed by OkHttp, a deserialization
 * step using Gson, rate limiting and a retry policy.
 *
 * <p>
...
public class OkHttpPendingResult<T, R extends ApiResponse<T>> implements PendingResult<T>, Callback {
	 ...
	 * @param exceptionsAllowedToRetry
	...
	public T await() throws ApiException, IOException, InterruptedException {
		...
		if (retryCounter > 0) {
			...
			LOG.debug(String.format("Sleeping between errors for %dms (retry #%d, already slept %dms)", delayMillis,
					retryCounter, cumulativeSleepTime));
		...
		// This callback will be called on another thread, handled by the
	...
	private boolean shouldRetry(Response response) {
		return RETRY_ERROR_CODES.contains(response.code()) && cumulativeSleepTime < errorTimeOut
