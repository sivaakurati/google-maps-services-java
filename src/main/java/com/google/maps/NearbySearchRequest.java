...
/**
 * A <a href=
 * "https://developers.google.com/places/web-service/search#PlaceSearchRequests">Nearby
 * Search</a> request.
 */
public class NearbySearchRequest
	 ...
	 *
	 * @param context
	 *            The {@code GeoApiContext} to make requests through.
	 */
	 ...
	 *
	 * @param location
	 *            The location to use as the center of the Nearby Search.
	...
	/**
	 * Specifies the distance (in meters) within which to return place results. The
	 * maximum allowed radius is 50,000 meters. Note that radius must not be
	 * included if {@code rankby=DISTANCE} is specified.
	 *
	 * @param distance
	 *            The distance in meters around the {@link #location(LatLng)} to
	 *            search.
	 * @return Returns this {@code NearbyApiRequest} for call chaining.
	 */
	...
	/**
	 * Specifies a term to be matched against all content that Google has indexed
	 * for this place. This includes but is not limited to name, type, and address,
	 ...
	 *
	 * @param priceLevel
	 *            The price level to set as minimum.
	 * @return Returns this {@code NearbyApiRequest} for call chaining.
	 */
	 ...
	 *
	 * @param priceLevel
	 *            The price level to set as maximum.
	 * @return Returns this {@code NearbyApiRequest} for call chaining.
	 */
	...
	/**
	 * Specifies one or more terms to be matched against the names of places,
	 * separated by spaces.
	 *
	 * @param name
	...
	/**
	 * Restricts to only those places that are open for business at the time the
	 * query is sent.
	 *
	 * @param openNow
	 *            Whether to restrict to places that are open.
	 * @return Returns this {@code NearbyApiRequest} for call chaining.
	 */
	...
	/**
	 * Returns the next 20 results from a previously run search. Setting
	 * {@code pageToken} will execute a search with the same parameters used
	 * previously — all parameters other than {@code
	 ...
	 *
	 * @param type
	 *            The {@link PlaceType} to restrict results to.
	 * @return Returns this {@code NearbyApiRequest} for call chaining.
	 */
	...
	/**
	 * Restricts the results to places matching the specified type. Provides support
	 * for multiple types.
	 *
	 * @deprecated Multiple search types are ignored by the Places API.
	 * @param types
	 *            The {@link PlaceType}s to restrict results to.
	 * @return Returns this {@code NearbyApiRequest} for call chaining.
	 */
	@Deprecated
	...
	protected void validateRequest() {
		...
		if (params().containsKey("rankby") && params().get("rankby").get(0).equals(RankBy.DISTANCE.toString())
		...
		// is required.
		if (params().containsKey("rankby") && params().get("rankby").get(0).equals(RankBy.DISTANCE.toString())
				&& !params().containsKey("keyword") && !params().containsKey("name") && !params().containsKey("type")) {
