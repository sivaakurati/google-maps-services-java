 ...
 *
 * GeoApiContext works best when you create a single GeoApiContext instance, or
 * one per API key, and reuse it for all your Google Geo API queries. This is
 * because each GeoApiContext manages its own thread pool, back-end client, and
 * other resources.
 *
 * <p>
 * When you are finished with a GeoApiContext object, you must call
 * {@link #shutdown()} on it to release its resources.
 */
public class GeoApiContext {
	...
	/* package */
	GeoApiContext(RequestHandler requestHandler, String apiKey, String baseUrlOverride, String channel, String clientId,
			long errorTimeout, ExceptionsAllowedToRetry exceptionsAllowedToRetry, Integer maxRetries,
			UrlSigner urlSigner) {
	...
	/**
	 * The service provider interface that enables requests to be handled via
	 * switchable back ends. There are supplied implementations of this interface
	 * for both OkHttp and Google App Engine's URL Fetch API.
	 *
	...
	/**
	 * Shut down this GeoApiContext instance, reclaiming resources. After shutdown()
	 * has been called, no further queries may be done against this instance.
	 */
	...
	<T, R extends ApiResponse<T>> PendingResult<T> get(ApiConfig config, Class<? extends R> clazz,
		...
		for (Map.Entry<String, List<String>> param : params.entrySet()) {
			...
			for (String value : values) {
				...
				try {
					...
					// This should never happen. UTF-8 support is required for every Java
					// implementation.
		...
		return getWithPath(clazz, config.fieldNamingPolicy, config.hostName, config.path, config.supportsClientId,
				query.toString());
	}
	...
	<T, R extends ApiResponse<T>> PendingResult<T> get(ApiConfig config, Class<? extends R> clazz, String... params) {
		...
		// Channel can be supplied per-request or per-context. We prioritize it from the
		// request,
	...
	<T, R extends ApiResponse<T>> PendingResult<T> post(ApiConfig config, Class<? extends R> clazz,
			Map<String, List<String>> params) {
		...
		return requestHandler.handlePost(hostName, url.toString(), params.get("_payload").get(0), USER_AGENT, clazz,
				config.fieldNamingPolicy, errorTimeout, maxRetries, exceptionsAllowedToRetry);
	}

	private <T, R extends ApiResponse<T>> PendingResult<T> getWithPath(Class<R> clazz,
			FieldNamingPolicy fieldNamingPolicy, String hostName, String path, boolean canUseClientId,
			String encodedPath) {
	...
	public static class Builder {
		 ...
		 *
		 * @param builder
		 *            The {@code RequestHandler.Builder} to use for {@link #build()}
		 ...
		 *
		 * @param baseUrl
		 *            The URL to use, without a trailing slash, e.g.
		...
		/**
		 * Sets the default channel for requests (can be overridden by requests). Only
		 * useful for Google Maps for Work clients.
		 *
		 * @param channel
		 *            The channel to use for analytics
		 * @return Returns this builder for call chaining.
		 */
		 ...
		 * @param timeout
		 *            The connect timeout period in {@code unit}s.
		 * @param unit
		 *            The connect timeout time unit.
		 * @return Returns this builder for call chaining.
		 */
		 ...
		 * @param timeout
		 *            The read timeout period in {@code unit}s.
		 * @param unit
		 *            The read timeout time unit.
		 * @return Returns this builder for call chaining.
		 */
		 ...
		 *
		 * @param timeout
		 *            The write timeout period in {@code unit}s.
		 * @param unit
		 *            The write timeout time unit.
		 * @return Returns this builder for call chaining.
		 */
		 ...
		 * Defaults to 60 seconds. Set to zero to retry requests forever.
		 *
		 * <p>
		 * This operates separately from the count-based {@link #maxRetries(Integer)}.
		 *
		 * @param timeout
		 *            The retry timeout period in {@code unit}s.
		 * @param unit
		 *            The retry timeout time unit.
		 * @return Returns this builder for call chaining.
		 */
		...
		/**
		 * Sets the maximum number of times each retry-able errors will be retried. Set
		 * this to null to not have a max number. Set this to zero to disable retries.
		 *
		 * <p>
		 * This operates separately from the time-based
		 * {@link #retryTimeout(long, TimeUnit)}.
		 *
		 * @param maxRetries
		...
		/**
		 * Disables retries completely, by setting max retries to 0 and retry timeout to
		 * 0.
		 *
		 * @return Returns this builder for call chaining.
		 */
		...
		/**
		 * Sets the maximum number of queries that will be executed during a 1 second
		 * interval. The default is 50. A minimum interval between requests will also be
		 * enforced, set to 1/(2 * {@code maxQps}).
		 *
		 * @param maxQps
		 ...
		 *
		 * @param exception
		 *            The {@code ApiException} to allow or deny being re-tried.
		 * @param allowedToRetry
		 *            Whether to allow or deny re-trying {@code exception}.
		 * @return Returns this builder for call chaining.
		 */
		...
		public GeoApiContext build() {
			return new GeoApiContext(builder.build(), apiKey, baseUrlOverride, channel, clientId, errorTimeout,
					exceptionsAllowedToRetry, maxRetries, urlSigner);
		}
	}
}
