...
public class FindPlaceFromTextRequest
		extends PendingResultBase<FindPlaceFromText, FindPlaceFromTextRequest, FindPlaceFromTextRequest.Response> {

	static final ApiConfig API_CONFIG = new ApiConfig("/maps/api/place/findplacefromtext/json")
			.fieldNamingPolicy(FieldNamingPolicy.LOWER_CASE_WITH_UNDERSCORES).supportsClientId(false);
	...
	public enum InputType implements UrlValue {
		TEXT_QUERY("textquery"), PHONE_NUMBER("phonenumber");
	 ...
	 *
	 * @param locationBias
	 *            The location bias for this request.
	 * @return Returns {@code FindPlaceFromTextRequest} for call chaining.
	 */
	...
	public enum FieldMask implements UrlValue {
		FORMATTED_ADDRESS("formatted_address"), GEOMETRY("geometry"), ICON("icon"), ID("id"), NAME(
				"name"), OPENING_HOURS("opening_hours"), PERMANENTLY_CLOSED("permanently_closed"), PHOTOS(
						"photos"), PLACE_ID("place_id"), PRICE_LEVEL("price_level"), RATING("rating"), TYPES("types");
